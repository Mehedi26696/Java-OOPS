...
// The default priority of the thread is 5 (NORM_PRIORITY). The minimum priority of the thread is 1 (MIN_PRIORITY) and the maximum priority of the thread is 10 (MAX_PRIORITY).

// The thread scheduler uses the priority of the thread to decide which thread will run. The thread with the highest priority will get the chance to run first.

// If two threads have the same priority then the thread scheduler uses the first come first serve algorithm to decide which thread will run first.
